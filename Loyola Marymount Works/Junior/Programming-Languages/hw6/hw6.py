...
class Rename:
  ...
  def __init__(self, in_headers, args):
    ...
    for ln in self.output_headers:
      if ln == self.args[0]:
        self.output_headers[self.output_headers.index(ln)] = self.args[1]
    self.aggregate_headers = []

  def process_row(self,row):
    row[self.args[1]] = row.pop(self.args[0])
    return row
...
class Swap:
    ...
    def __init__(self, in_headers, args):
      ...
      self.input_headers = in_headers
      self.output_headers = list(self.input_headers)
      if self.output_headers[0] == self.args[0]:
        a, b = self.output_headers.index(self.args[0]), self.output_headers.index(self.args[1])
        self.output_headers[a], self.output_headers[b] = self.output_headers[b], self.output_headers[a]
      self.aggregate_headers = []
...
class Select:
    ...
    def __init__(self, in_headers, args):
      ...
      self.input_headers = in_headers
      self.output_headers = list(self.input_headers)
      l3 = [x for x in self.output_headers if x not in self.args]
      for ln in l3:
        (self.output_headers).remove(ln)
      self.aggregate_headers = []
...
class Update:
    ...
    def process_row(self,row):
      if self.args[0] not in self.output_headers:
        raise Exception("Not in input headers.")
      row[self.args[0]] = eval(self.args[1], row)
      return row
...
class Add:
  ...
  def __init__(self, in_headers, args):
    self.args = args
    if args[0] in in_headers:
      raise Exception("Already a column name")
    self.input_headers = in_headers
    self.output_headers = in_headers
    if self.args[0] not in self.output_headers:
      (self.output_headers).append(self.args[0])
    self.aggregate_headers = []
...
class MaxBy:
    ...
    def __init__(self, in_headers, args):
      ...
      self.input_headers = in_headers
      self.output_headers = in_headers
      self.aggregate_headers = ["Max %s By %s" % (self.args[0], self.args[1])]
      self.max = 0
      self.name = ""

    def process_row(self,row):
      if float(row[self.args[1]]) > self.max:
        self.max = float(row[self.args[1]])
        self.name = row[self.args[0]]
      return row
...
class Sum:
    ...
    def __init__(self, in_headers, args):
      ...
      self.input_headers = in_headers
      self.output_headers = in_headers
      self.aggregate_headers = ["%s Sum" % self.args[0]]
      self.sum = 0

    def process_row(self,row):
      self.sum += int(row[self.args[0]])
      return row
...
class Mean:
    ...
    def __init__(self, in_headers, args):
      ...
      self.input_headers = in_headers
      self.output_headers = in_headers
      self.aggregate_headers = ["%s Mean" % self.args[0]]
      self.mean = 0
      self.numRows = 0

    def process_row(self,row):
      self.mean += int(row[self.args[0]])
      self.numRows += 1
      return row

    def get_aggregate(self):
      return {self.aggregate_headers[0]:round((self.mean)/self.numRows)}
...
class ComposeQueries:
    ...
    def process_row(self,row):
      if self.q1.process_row(row):
        return self.q2.process_row(self.q1.process_row(row))
      return None

    def get_aggregate(self):
      if self.q1.get_aggregate():
        if self.q2.get_aggregate():
          return self.q1.get_aggregate().update(self.q2.get_aggregate())
        else:
          return self.q1.get_aggregate()
      elif self.q2.get_aggregate():
        return self.q2.get_aggregate()
      else:
        return {}  
...
def splitArgs(args):
  ...
  for index, arg in enumerate(args):
    if '-' in arg:
      bigList.append(args[index:index+3])
  return bigList
...
def buildQuery(in_headers, args):
  ...
  result = queries[(argsList[0][0][1:])](in_headers, [argsList[0][1], argsList[0][2]])
  for arg in argsList:
    if '-' in arg[0]:
      query1 = queries[arg[0][1:]]
      result = ComposeQueries(result, query1(in_headers, argsList))
  return result
  
# test:
  """
  in_headers = (open('player_career_short.csv')).readline().strip().split(',')
...
def mostPointsPerMinute():
  ...
  """
  queryFile('player_career.csv', (['-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'pointsPerMinute', 'float(pts)/float(minutes) if float(minutes) > 0 else 0', '-MaxBy', 'fullname', 'pointsPerMinute']))
...
def mostStealsPerGame():
  ...
  """
  queryFile('player_career.csv', (['-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'mostStealsPerGame', 'float(stl)/float(gp)', '-MaxBy', 'fullname', 'mostStealsPerGame']))
...
def bestPercentages():
  ...
  percentage (tpm/tpa).
  """
  queryFile('player_career.csv', (['-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'highestFieldGoalPercentage', 'float(fgm)/float(fga)', '-MaxBy', 'fullname', 'highestFieldGoalPercentage', '-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'highestFreeThrowPercentage', 'float(ftm)/float(fta)', '-MaxBy', 'fullname', 'highestFreeThrowPercentage', '-Add', 'fullname', 'firstname + " " + lastname', '-Add', 'highestThreePointPercentage', 'float(tpm)/float(tpa)', '-MaxBy', 'fullname', 'highestThreePointPercentage']))

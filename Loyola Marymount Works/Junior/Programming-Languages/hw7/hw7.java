...
class Num implements AExp {
  ...
  public List<AInstr> compile() {
    LinkedList<AInstr> ans = new LinkedList<AInstr>();
    ans.push(new Push(val));
    return ans;
  }
...
class BinOp implements AExp {
  ...
  public List<AInstr> compile() {
    LinkedList<AInstr> ans = new LinkedList<AInstr>();
    ans.addAll(left.compile()); // returns list of instructions in left
    ans.addAll(right.compile());
...
class Calculate implements AInstr {
  ...
  //Problem 2
  public void eval(Stack<Double> stack) {
    Double ex1 = stack.pop();
    Double ex2 = stack.pop();
    stack.push(op.calculate(ex2, ex1));
  }
}

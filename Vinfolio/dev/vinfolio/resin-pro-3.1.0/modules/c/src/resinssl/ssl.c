...
struct connection_ops_t ssl_ops = {
  ssl_read,
  ssl_read, /* XXX: needs to be non-block */
  ssl_write,
  0,        /* write_nonblock */
  ssl_close,
...
ssl_create_context(JNIEnv *env, ssl_config_t *config)
{
  ...
  if (! config->password) {
    resin_printf_exception(env, "com/caucho/config/ConfigException", "password is required for <openssl>.");
    return 0;
  }
  ...
  if (config->cipher_suite &&
      ! SSL_CTX_set_cipher_list(ctx, config->cipher_suite)) {
...
ssl_read(connection_t *conn, char *buf, int len, int timeout)
{
  ...
  } while (retry-- > 0 && (errno == EINTR || errno == EAGAIN));
